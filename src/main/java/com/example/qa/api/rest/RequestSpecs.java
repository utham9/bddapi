...
public enum RequestSpecs {
    ...
    public static RequestSpecs getRequestConfig(String value) {
        return Arrays.stream(RequestSpecs.values()).filter(requestSpecs -> requestSpecs.restAssuredKey.equalsIgnoreCase(value))
                .findFirst().orElseThrow(() -> new TestException("%s is not a valid rest config", value));
    }
